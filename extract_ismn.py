"""
Script for extracting ISMN station data for any network or sensor type into
a dictionary stored in a pkl file, with data at a consistent hourly cadensce.
"""
...
def _preprocess_station_data(
        station_root_path:Path, station_dict:dict, station_pkl_dir:Path,
        var_mapping:dict={}, skip_existing=True):
    """
    ...
    :@param station_root_path: Unzipped directory from the ISMN downloader.
    :@param station_dict: Dictionary for this network/station combo containing
        fields describing the sensors, location, and metadata. All this info
        is extracted from the metadata returned by the ISMN_Interface.
        The dict should have the following fields:
         - network: str
         - station: str
         - sensors: list of dicts from metadata
         - location: 2-tuple coordinates
         - station_meta: dict from metadata
    :@param station_pkl_dir: Directory where station pkls generated by this
        method will be dumped.
    :@param var_mapping: Optional dictionary mapping ISMN variable names to
        new unique strings (since the ISMN names are so verbose)
    """
    assert station_pkl_dir.exists(), \
            f"{station_pkl_dir.as_posix()} must exist."
    stn = station_dict
    ...
    for ssr in stn["sensors"]:
        cur_var = var_mapping.get(ssr["variable"], ssr["variable"])
    ...
    station_pkl_dict = {
            ...
            "texture":np.stack([
                np.array(stn["station_meta"].get(
                    "sand_fraction", np.full(3, np.nan)
                    )),
                np.array(stn["station_meta"].get(
                    "silt_fraction", np.full(3, np.nan)
                    )),
                np.array(stn["station_meta"].get(
                    "clay_fraction", np.full(3, np.nan)
                    )),
                ], axis=-1),
...
def parse_ismn_stm(stm_path:Path, return_epoch_times=False, debug=False):
    ...
    if debug:
        dt = np.diff(etimes)
        print(
            f"{etimes.size = } {np.amin(dt)=}, {np.amax(dt)=}, " + \
            f"{np.average(dt)=}\n{np.count_nonzero(dt < 3599)=}, " + \
            f"{np.count_nonzero(dt > 3601)=}")
...
if __name__=="__main__":
    ...
    ## number of stations to extract in parallel
    nworkers = 8

    ## subset of sensor networks to utilize
    extract_networks = [
        "TxSON", "SOILSCAPE", "SNOTEL", "SCAN", "RISMA",
    ...
    args = [{
        ...
        "skip_existing":True,
        "station_root_path":ismn_stations_path,
        } for s in stations
        ]
    ...
    ## print some information on extracted stations as a sanity check
    #'''
    for ppath in station_pkl_dir.iterdir():
        with ppath.open("rb") as fp:
            sdict = pkl.load(fp)

            ## timesteps with all sensors valid
            ...
            print()
            print(sdict["network"], sdict["station"])
            print("Timesteps with all sensors valid: "
                f"{np.count_nonzero(m_all_valid)}/{m_all_valid.shape[0]}")
    #'''
